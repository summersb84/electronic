...
# ---------------------------------------------------------
# 제품 분석 하단: 카테고리 - 제품 계층별 매출 Treemap 차트
# ---------------------------------------------------------
st.markdown("---")
st.markdown("#### 🌳 카테고리-제품 계층별 매출 분포 (Treemap Analysis)")

# 1) 필요한 컬럼 감지 (Category, Product Name, Sales)
cat_cols = [c for c in filtered_df.columns if 'cat' in c.lower() or 'group' in c.lower() or 'type' in c.lower()]
prod_cols = [c for c in filtered_df.columns if 'prod' in c.lower() or 'item' in c.lower() or 'name' in c.lower()]

if prod_cols and 'Total Sales' in filtered_df.columns:
    prod_col = prod_cols[0]
    
    # 카테고리 컬럼이 없는 경우 임의의 'All Products' 카테고리로 생성하여 계층화
    if cat_cols:
        cat_col = cat_cols[0]
        ...
        cat_col = 'Category'
        tree_df = filtered_df.copy()
        tree_df['Category'] = '전체 제품군'

    # 수량/주문건수 컬럼 감지
    qty_cols = [c for c in filtered_df.columns if 'qty' in c.lower() or 'quantity' in c.lower() or 'count' in c.lower()]
    qty_col = qty_cols[0] if qty_cols else None

    # 2) 계층별 데이터 집계
    if qty_col:
        treemap_data = tree_df.groupby([cat_col, prod_col]).agg(
            Total_Sales=('Total Sales', 'sum'),
        ...
        treemap_data = tree_df.groupby([cat_col, prod_col]).agg(
            Total_Sales=('Total Sales', 'sum'),
            Total_Qty=('Total Sales', 'count')
        ).reset_index()

    # 3) Plotly Treemap 생성
    fig_treemap = px.treemap(
        treemap_data,
        path=[cat_col, prod_col],       # 계층 구조: 카테고리 -> 제품명
        values='Total_Sales',           # 면적 크기: 총 매출액
        color='Total_Sales',            # 색상 짙이: 매출액 금액
        color_continuous_scale='Blues', # 색상 팔레트 (블루 계열)
        title='Sales Breakdown by Category & Product'
    )

    # 차트 호버 툴팁 및 텍스트 레이아웃 설정
    fig_treemap.update_traces(
        hovertemplate='<b>%{label}</b><br>총 매출액: $%{value:,.2f}<extra></extra>',
        textinfo="label+value+percent parent"  # 상위 항목 대비 비중(%) 자동 표시
    )

    fig_treemap.update_layout(
        height=480,
        margin=dict(l=10, r=10, t=40, b=10),
        coloraxis_showscale=False  # 범례 바 숨김 (깔끔한 UI)
    )

    st.plotly_chart(fig_treemap, use_container_width=True)

    # 간단한 인사이트 안내
    st.caption("💡 **Tip:** 각 사각형의 면적은 전체 매출에서 차지하는 비중을 의미합니다. 영역을 클릭하면 해당 카테고리 내부 제품들의 매출 비중으로 드릴다운(Drill-down)하여 확인하실 수 있습니다.")

else:
    st.warning("Treemap 분석에 필요한 제품명 컬럼 또는 매출(Total Sales) 데이터가 부족합니다.")
